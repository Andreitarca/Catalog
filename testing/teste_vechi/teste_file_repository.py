...
def teste_lista_file_studenti():
    lista_1 = ListaStudentiFile("testing/test_files/teste_repository_studenti.txt")
    lista_1.del_all()
...
def teste_lista_file_discipline():
    lista_1 = ListaDisciplineFile("testing/test_files/teste_repository_discipline.txt")
    lista_1.del_all()
    assert lista_1.getlista() == {}
    assert lista_1.getlenlista() == 0
...
def teste_lista_file_note():
    ...
    Stud3 = Student(8, "Ioana")
    Stud5 = Student(10, "Cezar")
    repo_studenti = ListaStudentiFile("testing/test_files/teste_repository_studenti.txt")
    repo_discipline = ListaDisciplineFile("testing/test_files/teste_repository_discipline.txt")
    repo_studenti.del_all()
    repo_discipline.del_all()
    list = ListaNoteFile("testing/test_files/teste_repository_note.txt", repo_studenti, repo_discipline)
    list.del_all()
    assert list.getlenlista() == 0
    assert list.getlista() == {}
    repo_studenti.adauga_student(Stud1)
    repo_studenti.adauga_student(Stud5)
    repo_studenti.adauga_student(Stud2)
    repo_discipline.adauga_disciplina(Disc6)
    repo_discipline.adauga_disciplina(Disc7)
    repo_discipline.adauga_disciplina(Disc8)
...
def ruleaza_toate_testele_file_repository():
    teste_lista_file_studenti()
    teste_lista_file_discipline()
    teste_lista_file_note()
